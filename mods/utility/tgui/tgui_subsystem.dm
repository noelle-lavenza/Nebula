...
SUBSYSTEM_DEF(tgui)
	...
	/// A list of open UIs
	var/list/open_tguis = list()
	/// A list of open UIs, grouped by src_object.
	var/list/open_tguis_by_src = list()
...
/datum/controller/subsystem/tgui/PreInit()
	...
	basehtml = replacetext(basehtml, "tgui:stylesheet", "Nanotrasen (c) 2206")

/datum/controller/subsystem/tgui/Shutdown()
	close_all_uis()

/datum/controller/subsystem/tgui/stat_entry(msg)
	msg = "P:[length(open_tguis)]"
	return ..()

/datum/controller/subsystem/tgui/fire(resumed = FALSE)
	if(!resumed)
		src.current_run = open_tguis.Copy()
	...
	while(length(current_run))
		...
		if(ui?.user && ui.src_object)
			ui.process(wait / (1 SECOND))
		else
			open_tguis.Remove(ui)
		if(MC_TICK_CHECK)
			return
...
/datum/controller/subsystem/tgui/proc/get_open_ui(mob/user, datum/src_object)
	...
	if(isnull(open_tguis_by_src[key]) || !istype(open_tguis_by_src[key], /list))
		return null
	for(var/datum/tgui/ui in open_tguis_by_src[key])
...
/datum/controller/subsystem/tgui/proc/close_all_uis()
	var/count = 0
	for(var/key in open_tguis_by_src)
		for(var/datum/tgui/ui in open_tguis_by_src[key])
			// Check if UI is valid.
			if(ui?.src_object && ui.user && ui.src_object.ui_host(ui.user))
				ui.close()
...
/datum/controller/subsystem/tgui/proc/on_open(datum/tgui/ui)
	...
	if(isnull(open_tguis_by_src[key]) || !istype(open_tguis_by_src[key], /list))
		open_tguis_by_src[key] = list()
	ui.user.tgui_open_uis |= ui
	var/list/uis = open_tguis_by_src[key]
	uis |= ui
	open_tguis |= ui
