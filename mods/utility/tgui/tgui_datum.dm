 ...
 *
 * required state datum/topic_state/state Next state
 */
/datum/tgui/proc/set_state(datum/topic_state/state)
	src.state = state
 ...
 *
 * optional custom_data list Custom data to send instead of tgui_data.
...
/datum/tgui/proc/get_payload(custom_data, with_data, with_static_data)
	...
	json_data["config"] = list(
		...
		"window" = list(
			"key" = window_key,
			"size" = window_size,
			"locked" = user.get_preference_value(/datum/client_preference/tgui_monitor) == PREF_PRIMARY,
			"scale" = TRUE
		),
		"client" = list(
...
/datum/tgui/proc/process_status()
	...
	status = src_object.CanUseTopic(user, state)
...
#undef COOLDOWN_DECLARE
#undef COOLDOWN_FINISHED
#undef COOLDOWN_TIMELEFT
#undef COOLDOWN_START