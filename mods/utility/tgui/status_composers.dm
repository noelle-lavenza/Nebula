...
/proc/ui_status_user_is_advanced_tool_user(mob/user)
	return user.IsAdvancedToolUser() ? STATUS_INTERACTIVE : STATUS_UPDATE
...
/mob/living/silicon/proc/get_ui_access(atom/source)
	return STATUS_INTERACTIVE // Ubiquitous networking. Do not abuse.

/// Return STATUS_INTERACTIVE if the user is strictly adjacent to the target atom, whether they can see it or not.
/// Return STATUS_CLOSE otherwise.
