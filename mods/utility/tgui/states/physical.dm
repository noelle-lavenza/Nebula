...
GLOBAL_DATUM_INIT(physical_obscured_state, /datum/topic_state/physical_obscured_state, new)

/datum/topic_state/physical_obscured_state/can_use_topic(src_object, mob/user)
	. = user.shared_ui_interaction(src_object)
	if(. > STATUS_CLOSE)
...
/mob/living/silicon/physical_obscured_can_use_topic(src_object)
	return max(STATUS_UPDATE, shared_living_ui_distance(src_object, viewcheck = FALSE)) // Silicons can always see.
