 ...
 * * ui_state - The TGUI UI state that will be returned in ui_state(). Default: interactive_topic_state
 */
/proc/tgui_input_number(mob/user, message, title = "Number Input", default = 0, max_value = 10000, min_value = 0, timeout = 0, integer_only = TRUE, ui_state = global.interactive_topic_state)
	if (!user)
		user = usr
	if (!istype(user))
...
/proc/tgui_input_real_number(mob/user, message, title = "Number Input", default = 0, max_value = -10000, min_value = -10000, timeout = 0, integer_only = FALSE, ui_state = global.interactive_topic_state)
 ...
 * * ui_state - The TGUI UI state that will be returned in ui_state(). Default: interactive_topic_state
 */
/proc/tgui_input_number_async(mob/user, message, title = "Number Input", default  = 0, max_value = 10000, min_value  = 0, datum/callback/callback, timeout = 60 SECONDS, integer_only = TRUE, ui_state = global.interactive_topic_state)
	if (!user)
		user = usr
	if (!istype(user))
		if (istype(user, /client))
...
/datum/tgui_input_number
	...
	var/title
	/// The TGUI UI state that will be returned in ui_state(). Default: interactive_topic_state
...
/datum/tgui_input_number/ui_act(action, list/params, datum/tgui/ui, datum/topic_state/state)
	. = ..()
	if (.)
		return
	switch(action)
