 ...
 */
/proc/tgui_alert(mob/user, message = "", title, list/buttons = list("Ok"), timeout = 60 SECONDS, autofocus = TRUE, ui_state = global.interactive_topic_state)
	if (!user)
		user = usr
	if (!istype(user))
 ...
 * * ui_state - The TGUI UI state that will be returned in ui_state(). Default: interactive_topic_state
 */
/proc/tgui_alert_async(mob/user, message = "", title, list/buttons = list("Ok"), datum/callback/callback, timeout = 60 SECONDS, autofocus = TRUE, ui_state = global.interactive_topic_state)
	if (!user)
		user = usr
	if (!istype(user))
		if (istype(user, /client))
...
/datum/tgui_modal/ui_act(action, list/params, datum/tgui/ui, datum/topic_state/state)
	. = ..()
	if (.)
		return
	switch(action)
